  ...
  (() => {
    ...
    function dotColor(dx, dy, n, edge, p) {
      ...
      // ensure haldi appears even in "creamy" patterns:
    ...
    // Subtle: just a few dots near the center, stamped 8-way.
    function stampPaletteSignature(cx, cy, w, h, p) {
      const s = p.spacing;
      // pick three nearby octant coords (dy <= dx) based only on seed/spacing
      const a = 1 + (hash2(101, 102, p.seed) % 2); // 1..2
      const b = 1 + (hash2(103, 104, p.seed) % 2); // 1..2
    ...
    function renderWithParams(p) {
      ...
      for (let dx = 0; dx <= Math.max(maxDx, maxDy); dx++) {
        for (let dy = 0; dy <= dx; dy++) {
          ...
          // stitch edges (still deterministic + symmetric)
          const edge = Math.abs(Math.sin(v)) < p.edgeWidth;

          // Dotty bandhani: dots live on one (or two) lattices.
          // Second lattice is enabled deterministically (via hash).
          ...
          // Allow edge-stitch dots even off-lattice, but sparsely (keeps dotty feel)
          ...
          // Slight edge fade to keep canvas edges cleaner
      ...
      // Ensure all three colors appear in every pattern
      stampPaletteSignature(cx, cy, w, h, p);
    }

    // --- Tests (keep them lightweight but meaningful) ---
    function validateGenerator(){
      ...
      for (let i = 0; i < 10; i++) {
        const dx = 3 + i;
        const dy = i % 3;
        const a1 = latticeA(dx, dy, p);
        const a2 = latticeA(dx, dy, p);
        assert(a1 === a2, 'latticeA() is not deterministic.');
      }

      // Added test: dotColor must be capable of producing BOTH cream and haldi
      // for some deterministic inputs.
      const c1 = dotColor(5, 1, 0.999, false, p);
      const c2 = dotColor(6, 2, 0.900, false, p);
      assert([CREAM, HALDI].includes(c1), 'dotColor() did not return cream/haldi for high n.');
      assert([CREAM, HALDI, null].includes(c2), 'dotColor() returned an unexpected value.');
    }
